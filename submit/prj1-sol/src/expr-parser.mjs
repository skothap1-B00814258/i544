...
class ExprParser {
  ...
  match(type) {
    if (this.tok.type === type) {
      if (this.tok.type !== '<EOF>') this.nextTok();
    }
    else {
...
function scan(str, baseCell) {
 ...
 // console.log(tok);
  ...
  tokens.push({ type: '<EOF>', lexeme: '<EOF>', value: '', });
  // console.log(tokens);
  return tokens;
}
...
class CellRef {
  constructor(str='', baseCell=null) {
    // console.log("base-cell",baseCell);
    ...
    if (str.length === 0) {
      ...
      // console.log("match in cell ref", match);
      if (!match) {
